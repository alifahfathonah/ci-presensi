...
$(document).ready(function () {
  ...
  table = $('#table_jeniskantor').DataTable({
    "lengthChange": true,
		"lengthMenu": [
			[10, 25, 50, -1],
			[10, 25, 50, "All"]
		...
		"ajax": {
			"url": base_url + "jeniskantor/ajax_list/",
			"type": "POST"
		},
...
function form_validation() {
	$('#form-jeniskantor').on('submit', function (event) {
		event.preventDefault();
		event.stopPropagation();

		var input_list = [
			'input_nama_jeniskantor'
		];

		var input_list_error = [
			'input_nama_jeniskantor_error_detail'
		];

		$.ajax({
			url: base_url + "jeniskantor/validation/",
			method: 'POST',
			data: $(this).serialize(),
			dataType: 'json',
			beforeSent: function () {
...
function save() {
	var url;
	if (save_method == "add") {
		url = base_url + "jeniskantor/insert/";
	} else {
		url = base_url + "jeniskantor/update/";
	}
	var form = $('#form-jeniskantor')[0];
	...
	$.ajax({
		...
		processData: false,
		success: function (data) {
      window.open(base_url + "jeniskantor", "_self");
      $('#form-jeniskantor')[0].reset();
		}
	});
}
...
function hapus_data(id) {
	if (confirm('Anda yakin akan menghapus data ini?')) {
		$.ajax({
			url: base_url + "jeniskantor/delete/" + id,
			method: "GET",
			dataType: 'json',
			success: function (data) {
				if (data.status) {
