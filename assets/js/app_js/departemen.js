...
$(document).ready(function () {
  ...
  table = $('#table_departemen').DataTable({
    "lengthChange": true,
		"lengthMenu": [
			[10, 25, 50, -1],
			[10, 25, 50, "All"]
...
function form_validation() {
	$('#form-departemen').on('submit', function (event) {
		event.preventDefault();
		event.stopPropagation();

		var input_list = [
			'input_nama_departemen',
			'input_kode_departemen',
		];

		var input_list_error = [
			'input_nama_departemen_error_detail',
			'input_kode_departemen_error_detail'
		];

		$.ajax({
			url: base_url + "departemen/validation/",
			method: 'POST',
			data: $(this).serialize(),
			dataType: 'json',
			beforeSent: function () {
...
function save() {
	var url;
	if (save_method == "add") {
		url = base_url + "departemen/insert/";
	} else {
		url = base_url + "departemen/update/";
	}
	var form = $('#form-departemen')[0];
	...
	$.ajax({
		...
		processData: false,
		success: function (data) {
      window.open(base_url + "departemen", "_self");
      $('#form-departemen')[0].reset();
		}
	});
}
