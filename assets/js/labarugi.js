...
function getdata(start_date, end_date) {
	...
	var prosesing = "processing";
	$('#content_labarugi').html(prosesing);
	$.ajax({
		url: base_url + "labarugi/getdata/" + start_date + "/" + end_date,
		method: 'GET',
		success: function (html) {
			$('#content_labarugi').html(html);
		}
	});
}
...
function reload_table() {
	window.open(base_url + 'labarugi', "_self");
}

function cetak_laporan() {
	window.open(base_url + 'labarugi/cetak/' + start_filter_date + "/" + end_filter_date, "_blank");
}