...
function fetch_data(start_date='', end_date=''){
  table = $('#table_pengeluarankas').DataTable({
    "lengthChange" : true,
    ...
    "buttons": ['colvis'],
    "ajax": {
      "url": base_url+"pengeluarankas/ajax_list/",
      "type": "POST",
      "data" : function (data) {
  ...
  $('#reportrange').on('apply.daterangepicker', function(ev, picker) {
    ...
    filter_date = true;
    $('#table_pengeluarankas').DataTable().destroy();
    fetch_data(start, end);
  });
}

function reload_table(){
  window.open(base_url + 'pengeluarankas', "_self");
}
...
function get_excel(){
  ...
  if(filter_date){
    ...
    url = base_url+"pengeluarankas/get_excel/" + "1/" +  start_filter_date + "/"+ end_filter_date;
  } else {
    search = $('[type="search"]').val();
    console.log(search);
    url = base_url+"pengeluarankas/get_excel/" + "2/" + search;
  }
  window.location = url;
}
...
function get_pdf(){
  var url = "";
  url = base_url+"pengeluarankas/getpdf/";
...
function export_excel(){
  ...
  $.ajax({
    ...
    url : base_url+"pengeluarankas/export/",
    success: function(html){
      // window.location = base_url+"pengeluarankas/export/";
      // console.log(html);
...
function isNumberKey(evt){
  ...
  function add(){
    $('#form_trans_pengeluaran_kas')[0].reset();
    save_method = "add";
    $('#modal_add_trans_pengeluaran_kas').modal('show');
  }

  function form_validation(){
    $('#form_trans_pengeluaran_kas').on('submit', function(event){
      event.preventDefault();
      event.stopPropagation();
      $.ajax({
        url: base_url + "pengeluarankas/validation/"+save_method,
        method: 'POST',
        data: $(this).serialize(),
        dataType: 'json',
        beforeSent:function(){
        ...
        success: function(data){
          if(data.error){
            ...
            if(data.input_untuk_akun_error != ""){
              $('#input_untuk_akun_error').html(data.input_untuk_akun_error);
              $('#input_untuk_akun').addClass('is-invalid');
            }else {
              $('#input_untuk_akun_error').html('');
              $('#input_untuk_akun').removeClass('is-invalid');
              $('#input_untuk_akun').addClass('is-valid');
            }
          ...
          if(data.success){
            ...
            $('#input_untuk_akun').removeClass('is-invalid');
            $('#input_untuk_akun').removeClass('is-valid');
            $('#input_untuk_akun_error').html('');
  ...
  function save(){
    ...
    if(save_method == "add") {
      url             = base_url + "pengeluarankas/save/";
      success_message = 'Successfully added ';
      error_message   = 'Insert data failed';
    } else {
      url             = base_url + "pengeluarankas/update/";
      success_message = 'Successfully updated ';
      error_message   = 'Update data failed';
    }

    var form = $('#form_trans_pengeluaran_kas')[0];
    ...
    $.ajax({
      ...
      success: function(data){
        if(data.status){
          $('#modal_add_trans_pengeluaran_kas').modal('hide');
          reload_table();
        }
      }
    });
  ...
  function edit(id){
    ...
    $('#form_trans_pengeluaran_kas')[0].reset();
    $.ajax({
        url : base_url+"pengeluarankas/ajax_detail/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="input_untuk_akun"]').val(data.jns_trans);
  ...
  function delete_data(id, kode){
    ...
    if(confirm('Apakah Anda akan menghapus data dengan kode: '+kode+' ?')){
      ...
      if (reason == null || reason == "") {
        ...
        url = base_url + "pengeluarankas/soft_delete/";
        $.ajax({
          url: url,
          method: 'POST',
