...
$(document).ready(function() {
  ...
  function fetch_data(start_date='', end_date=''){
    table = $('#table_simpanan').DataTable({
      "lengthChange" : true,
    ...
    $('#reportrange').on('apply.daterangepicker', function(ev, picker) {
      ...
      filter_date = true;
      $('#table_simpanan').DataTable().destroy();
      fetch_data(start, end);
    });
  }

  function reload_table(){
    window.open(base_url + 'simpanan', "_self");
  }
  ...
  function get_excel(){
    ...
    if(filter_date){
      ...
      url = base_url+"simpanan/get_excel/" + "1/" +  start_filter_date + "/"+ end_filter_date;
    } else {
      search = $('[type="search"]').val();
      console.log(search);
      url = base_url+"simpanan/get_excel/" + "2/" + search;
    }
    window.location = url;
  }
  //
  ...
  function export_excel(){
    ...
    $.ajax({
      ...
      url : base_url+"simpanan/export/",
      success: function(html){
        // window.location = base_url+"simpanan/export/";
        // console.log(html);
  ...
  function isNumberKey(evt){
    ...
    function add(){
      $('#form_trans_simpanan')[0].reset();
      save_method = "add";
      $('#modal_add_trans_simpanan').modal('show');
    }

    function form_validation(){
      $('#form_trans_simpanan').on('submit', function(event){
        event.preventDefault();
        event.stopPropagation();
        $.ajax({
          url: base_url + "simpanan/validation/"+save_method,
          method: 'POST',
          data: $(this).serialize(),
          dataType: 'json',
          beforeSent:function(){
          ...
          success: function(data){
            if(data.error){
              ...
              if(data.input_jumlah_simpanan_error != ""){
                $('#input_jumlah_simpanan_error').html(data.input_jumlah_simpanan_error);
                $('#input_jumlah_simpanan').addClass('is-invalid');
              }else {
                $('#input_jumlah_simpanan_error').html('');
                $('#input_jumlah_simpanan').removeClass('is-invalid');
                $('#input_jumlah_simpanan').addClass('is-valid');
              }
              ...
              if(data.input_simpan_ke_kas_error != ""){
                $('#input_simpan_ke_kas_error').html(data.input_simpan_ke_kas_error);
                $('#input_simpan_ke_kas').addClass('is-invalid');
              }else {
                $('#input_simpan_ke_kas_error').html('');
                $('#input_simpan_ke_kas').removeClass('is-invalid');
                $('#input_simpan_ke_kas').addClass('is-valid');
              }
            ...
            if(data.success){
              ...
              $('#input_jumlah_simpanan').removeClass('is-invalid');
              $('#input_jumlah_simpanan').removeClass('is-valid');
              $('#input_jumlah_simpanan_error').html('');
              ...
              $('#input_simpan_ke_kas').removeClass('is-invalid');
              $('#input_simpan_ke_kas').removeClass('is-valid');
              $('#input_simpan_ke_kas_error').html('');
    ...
    function save(){
      ...
      var form = $('#form_trans_simpanan')[0];
      ...
      $.ajax({
        ...
        success: function(data){
          if(data.status){
            $('#modal_add_trans_simpanan').modal('hide');
            reload_table();
          }
        }
      });
    ...
    function edit(id){
      ...
      $('#form_trans_simpanan')[0].reset();
      $.ajax({
        url : base_url+"simpanan/ajax_detail/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
          ...
          $('[name="input_nama_penyetor"]').val(data.nama_penyetor);
          $('[name="input_nomor_id_penyetor"]').val(data.no_identitas);
          $('[name="input_alamat_penyetor"]').val(data.alamat);
          $('[name="input_nama_anggota"]').val(data.nama);
          $('[name="input_jenis_simpanan"]').val(data.jenis_id);
          $('[name="input_jumlah_simpanan"]').val(data.jumlah);
          $('[name="input_keterangan"]').val(data.keterangan);
          $('[name="input_simpan_ke_kas"]').val(data.kas_id);
          $('#modal_add_trans_simpanan').modal('show');
    ...
    function delete_data(id, kode){
      ...
      if(confirm('Apakah Anda akan menghapus data dengan kode: '+kode+' ?')){
        ...
        if (reason == null || reason == "") {
          ...
          url = base_url + "simpanan/soft_delete/";
          $.ajax({
            url: url,
            method: 'POST',
