...
function getdata(start_date, end_date) {
    ...
    $('#content_neraca').html(prosesing);
	$.ajax({
		url: base_url + "neraca/getdata/" + start_date + "/" + end_date,
		method: 'GET',
		success: function (html) {
			$('#content_neraca').html(html);
		}
	});
}
...
function reload_table() {
	window.open(base_url + 'neraca', "_self");
}

function cetak_laporan() {
	window.open(base_url + 'neraca/cetak/' + start_filter_date + "/" + end_filter_date, "_blank");
}