...
function fetch_data(start_date='', end_date=''){
  table = $('#table_pemasukankas').DataTable({
    "lengthChange" : true,
    ...
    "buttons": ['colvis'],
    "ajax": {
      "url": base_url+"pemasukankas/ajax_list/",
      "type": "POST",
      "data" : function (data) {
  ...
  $('#reportrange').on('apply.daterangepicker', function(ev, picker) {
    ...
    filter_date = true;
    $('#table_pemasukankas').DataTable().destroy();
...
function reload_table(){
  ...
  //       '<b class="fa fa-filter"></b></div></div>'
  //     );
  // $("#table_pemasukankas").dataTable().fnDestroy();
  // filter_date = false;
  // fetch_data();
  window.open(base_url + 'pemasukankas', "_self");
}
...
function get_excel(){
  ...
  if(filter_date){
    ...
    url = base_url+"Pemasukankas/get_excel/" + "1/" +  start_filter_date + "/"+ end_filter_date;
  } else {
    search = $('[type="search"]').val();
    console.log(search);
    url = base_url+"Pemasukankas/get_excel/" + "2/" + search;
  }
  window.location = url;
}
...
function get_pdf(){
  var url = "";
  url = base_url+"Pemasukankas/getpdf/";
...
function export_excel(){
  ...
  $.ajax({
    ...
    url : base_url+"Pemasukankas/export/",
    success: function(html){
      // window.location = base_url+"Pemasukankas/export/";
      // console.log(html);
...
function isNumberKey(evt){
  ...
  function add(){
    $('#form_trans_pemasukan_kas')[0].reset();
    save_method = "add";
    $('#modal_add_trans_pemasukan_kas').modal('show');
  }

  function form_validation(){
    $('#form_trans_pemasukan_kas').on('submit', function(event){
      event.preventDefault();
      event.stopPropagation();
      $.ajax({
        url: base_url + "pemasukankas/validation/"+save_method,
        method: 'POST',
        data: $(this).serialize(),
        dataType: 'json',
        beforeSent:function(){
        ...
        success: function(data){
          if(data.error){
            ...
            if(data.input_dari_akun_error != ""){
              $('#input_dari_akun_error').html(data.input_dari_akun_error);
              $('#input_dari_akun').addClass('is-invalid');
            }else {
              $('#input_dari_akun_error').html('');
              $('#input_dari_akun').removeClass('is-invalid');
              $('#input_dari_akun').addClass('is-valid');
            }
          ...
          if(data.success){
            ...
            $('#input_dari_akun').removeClass('is-invalid');
            $('#input_dari_akun').removeClass('is-valid');
            $('#input_dari_akun_error').html('');
  ...
  function save(){
    ...
    if(save_method == "add") {
      url             = base_url + "pemasukankas/save/";
      success_message = 'Successfully added ';
      error_message   = 'Insert data failed';
    } else {
      url             = base_url + "pemasukankas/update/";
      success_message = 'Successfully updated ';
      error_message   = 'Update data failed';
    }

    var form = $('#form_trans_pemasukan_kas')[0];
    ...
    $.ajax({
      ...
      success: function(data){
        if(data.status){
          $('#modal_add_trans_pemasukan_kas').modal('hide');
          reload_table();
        }
      }
    });
  ...
  function edit(id){
    ...
    $('#form_trans_pemasukan_kas')[0].reset();
    $.ajax({
        url : base_url+"pemasukankas/ajax_detail/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="input_dari_akun"]').val(data.jns_trans);
            $('#modal_add_trans_pemasukan_kas').modal('show');
  ...
  function delete_data(id, kode){
    ...
    if(confirm('Apakah Anda akan menghapus data dengan kode: '+kode+' ?')){
      ...
      if (reason == null || reason == "") {
        ...
        url = base_url + "pemasukankas/soft_delete/";
        $.ajax({
          url: url,
          method: 'POST',
