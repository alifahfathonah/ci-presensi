...
class Pemasukankas extends CI_Controller{
  ...
  public function __construct(){
    parent::__construct();
    $this->load->model(array('Pemasukan_kas_model'));
  ...
  function index(){
    $this->template->load('Template', 'back/Pemasukankas/view_pemasukan_kas');
  }

  public function ajax_list(){
    // print_r($this->input->post());
    $list = $this->Pemasukan_kas_model->get_datatables();
    $data = array();
    $no = $_POST['start'];
    foreach ($list as $r) {
      $no++;
    ...
    $output = array(
      "draw" => $_POST['draw'],
      "recordsTotal" => $this->Pemasukan_kas_model->count_all(),
      "recordsFiltered" => $this->Pemasukan_kas_model->count_filtered(),
      "data" => $data,
    );
  ...
  function test(){
    ...
    $data = $this->Pemasukan_kas_model->get_all_data_by_time($period1, $period2)->result_array();
    print_r($data);
  }
  ...
  public function getpdf(){
    ...
    $pdf->Cell(277, 15, 'Laporan Data Pemasukan Kas', 0, 1, 'C');
    ...
    public function get_excel($type="", $param_1="", $param_2=""){
      ...
      if($type == '2'){
        if($param1 == ""){
          ...
          $data = $this->Pemasukan_kas_model->get_all_data();
        } else {
          $desc = "get all data hasil search";
          $data = $this->Pemasukan_kas_model->get_all_by($param1);
        }
      } else if($type == '1'){
        $desc = "gell all filter tanggal";
        $data = $this->Pemasukan_kas_model->get_all_by_date($param1, $param2);
      }

      $title = 'Data Pemasukan Kas KSU Sakrawarih - '.date('d-m-Y');
      ...
      $i=1; foreach($data->result() as $r) {
        ...
        echo '    <td>'.$r->dari_akun.'</td>';
        echo '    <td>'.$r->untuk_kas.'</td>';
    ...
    public function validation($method){
      ...
      $this->form_validation->set_rules('input_dari_akun', 'Dari Akun', 'callback_pilih_dari_akun');
      $this->form_validation->set_rules('input_untuk_kas', 'Untuk Kas', 'callback_pilih_untuk_kas');
      ...
      if($this->form_validation->run()){
        ...
        $array = array(
          ...
          'input_dari_akun_error' => form_error('input_dari_akun', '<b class="fa fa-warning"></b> ', ' '),
    ...
    public function save(){
      $object = array(
        ...
        'jns_trans' => $this->input->post('input_dari_akun'),
        'dk' =>'D',
        'user_name' => $this->session->userdata('username'),
        'is_del' => 0
      );
      $inserted_id = $this->Pemasukan_kas_model->insert($object);
      $json_return = array(
        'status'      => TRUE,
    ...
    function ajax_detail($id){
      $data = $this->Pemasukan_kas_model->get_by('id', $id);
    ...
    public function update(){
      ...
      $inserted_id = $this->Pemasukan_kas_model->update($object, $where);
      $json_return = array(
        'status'      => TRUE,
        'inserted_id' => $inserted_id,
      );
